        ...
        <div class="flex flex-col gap-3 mb-4 sm:flex-row sm:items-center">
            ...
            <label class="flex flex-col text-sm text-gray-700 dark:text-gray-300">
                ...
                <select v-model="selectedSeller"
                    ...
                    <option v-for="seller in sellerOptions" :key="seller" :value="seller">
                        {{ seller }}
                    </option>
                </select>
            </label>
            ...
            <table class="w-full text-left table-auto">
                ...
                <tbody>
                    <tr v-for="sale in filteredSales" :key="sale.id"
                        ...
                        <td class="px-5 py-3 text-gray-900 dark:text-gray-100">
                            {{ sale.product_name }}
                            <span class="text-sm text-gray-500 dark:text-gray-400">({{ sale.product_brand }})</span>
                        </td>
                        <td class="px-5 py-3 text-gray-900 dark:text-gray-100">
                            {{ sale.quantity_sold }}
                        </td>
                    ...
                    <tr v-if="filteredSales.length === 0">
                        <td colspan="7" class="py-6 italic text-center text-gray-500 dark:text-gray-400">
                            No sales found.
                        </td>
                    </tr>
                </tbody>
                <tfoot v-if="filteredSales.length > 0">
                    <tr class="font-semibold bg-gray-100 dark:bg-gray-800">
                        <td colspan="2" class="px-5 py-3 text-right text-gray-700 dark:text-gray-200">
                            Totals:
                        </td>
...
const sellerOptions = computed(() => {
    ...
    sales.value.forEach((sale) => {
        if (sale.seller_name) {
            uniqueSellers.add(sale.seller_name);
        }
    });
    ...
    confirmInternetAction(async () => {
        ...
        try {
            ...
            if (result.status === "success") {
                await Swal.fire(
                    "Synced!",
                    `${result.synced} sales uploaded to cloud.`,
                    "success"
                );
...
const filteredSales = computed(() => {
    return sales.value.filter((sale) => {
        ...
        const matchesDate =
            (!startDate.value ||
                createdAt.isSameOrAfter(dayjs(startDate.value), "day")) &&
            (!endDate.value || createdAt.isSameOrBefore(dayjs(endDate.value), "day"));

        const matchesSeller =
            !selectedSeller.value || sale.seller_name === selectedSeller.value;
