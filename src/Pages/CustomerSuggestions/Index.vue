...
async function syncToCloud() {
    loading.value = true;
    try {
        const result = await window.electronAPI.syncCustomerSuggestionsToCloud();
        if (result.status === "success") {
            Swal.fire(
                "Synced",
                `${result.synced} suggestions synced to the cloud`,
                "success"
            );
        } else if (result.status === "no_data") {
            Swal.fire("No Sync Needed", result.message, "info");
        } else {
            throw new Error(result.message || "Sync failed");
        }
        await fetchSuggestions();
    } catch (err) {
        Swal.fire("Error", err.message || "Failed to sync", "error");
    } finally {
        loading.value = false;
    }
}
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <div>
                    ...
                    <button @click="syncToCloud"
                        class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg transition ml-2">
                        Sync to Cloud
                    </button>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse rounded overflow-hidden">
                    <thead class="bg-gray-200 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-4 py-2 font-semibold text-sm text-center">
                                Synced
                            </th>
                            <!-- ✅ Added -->
                            <th class="px-4 py-2 font-semibold text-sm text-center">
