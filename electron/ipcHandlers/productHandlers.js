...
import syncEditedProductsToCloud from "../../logic/syncUpdatedProducts.js";
...
export default function registerProductHandlers() {
  ...
  ipcMain.handle("financials:get-latest", async () => {
    return await db("financial_summaries").orderBy("sync_at", "desc").first();
  });

  ipcMain.handle("summaries:get-latest", async () => {
    return await db("summaries").orderBy("synced_at", "desc").first();
  });

  ipcMain.handle("product-profits:get-latest", async () => {
    return await db("product_profits").orderBy("updated_at", "desc").first();
  });
    ...
    try {
      ...
      const results = await db("products")
        ...
        .andWhere(
          "expire_date",
          "<=",
          formatISO(limitDate, { representation: "date" })
        )
        .andWhere("quantity_remained", ">", 0)
        .orderBy("expire_date", "asc");
  ...
  ipcMain.handle("products:sync-to-cloud", async () => {
    return await syncEditedProductsToCloud();
  });
}
