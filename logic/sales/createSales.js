...
export default async function createSales(salesData) {
  ...
  await db.transaction(async (trx) => {
    ...
    for (const sale of salesData) {
      ...
      const profit =
        (price_per_unit - product.buying_price_per_unit) * quantity_sold;
      ...
      await products(trx).updateQuantity(
        product_id,
        product.quantity_remained - quantity_sold
      );
  ...
  try {
    ...
    const grandTotal = receiptProducts.reduce(
