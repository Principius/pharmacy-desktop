...
export default async function syncCustomerSuggestionsToCloud() {
  try {
    ...
    console.error('Customer Suggestions Sync Error:', error.response?.data || error.message)
    return {
      status: 'error',
      message: error.response?.data?.message || error.message || 'Failed to sync customer suggestions',
    }
  }
}
